...
Published release `v0.1.7` includes:

- public `linux/amd64` and `linux/arm64` image
  `ghcr.io/gaofeng21cn/ambient-ops:0.1.7`
...
`ghcr.io/gaofeng21cn/ambient-ops:0.1.7`; set `AMBIENT_OPS_IMAGE` in `.env` to
the reviewed release tag. The GHCR package is public, so a Docker host pulls it
anonymously:

```bash
docker pull ghcr.io/gaofeng21cn/ambient-ops:0.1.7
```
